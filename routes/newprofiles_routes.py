...
@newprofiles_routes_bp.route('/savenewprofile', methods=['POST'])
def savenewprofile():
    try:
        # Retrieve form data
        ...
        if connection_test_result is True:
            ...
            flash('Profile created and connection is valid!',
                  'success')  # Flash success message
            # Ensure this is the correct endpoint
            return redirect(url_for('newprofile.newprofiles'))
        else:
            # Connection test failed, flash error message
            flash(f'Connection test failed: {
                  connection_test_result}', 'danger')
            # Redirect back to profile creation page
            return redirect(url_for('newprofile.newprofiles'))
    except Exception as e:
        flash(f'An error occurred while saving the profile: {
              e}', 'danger')  # Use flash for general error
        # Redirect back to profile creation page
        return redirect(url_for('newprofile.newprofiles'))
